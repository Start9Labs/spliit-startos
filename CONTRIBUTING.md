...
This repo packages [Spliit](https://spliit.app/) for StartOS.
...
Spliit runs the upstream-published `ghcr.io/spliit-app/spliit` image. Upstream publishes multi-arch (amd64/arm64) images to GHCR via their own CI on every git tag, so we don't build anything ourselves. To track a new release:

1. Check available tags at <https://github.com/spliit-app/spliit/pkgs/container/spliit>.
2. Bump `images.main.source.dockerTag` in `startos/manifest/index.ts` to `ghcr.io/spliit-app/spliit:<new version>`.
...
If upstream stops publishing or an unreleased feature is needed, the previous approach (custom Dockerfile fetching a tarball at a pinned SHA) lives in git history at commit `791c8d7`.
